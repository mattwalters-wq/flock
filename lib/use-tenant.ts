                ...
                async function load() {
                          try {
                                      ...
                                      const tenantQuery = await (supabase
                                                                           ...
                                                                           .eq('slug', slug)
                                                                           .single() as unknown as Promise<{ data: TenantRow | null; error: { message: string } | null }>)

                            const tenantRow: TenantRow | null = tenantQuery.data
                            ...
                            const configQuery = await (supabase
                                                                 .from('tenant_config')
                                                                 .select('key, value')
                                                                 .eq('tenant_id', tenantRow.id) as unknown as Promise<{ data: ConfigRow[] | null; error: { message: string } | null }>)

                            const membersQuery = await (supabase
                                                                  .from('tenant_members')
                                                                  .select('id, slug, name, accent_color, bio, avatar_url, display_order')
                                                                  .eq('tenant_id', tenantRow.id) as unknown as Promise<{ data: MemberRow[] | null; error: { message: string } | null }>)

                            if (configQuery.error) throw new Error(configQuery.error.message)
                                      if (membersQuery.error) throw new Error(membersQuery.error.message)

                            // Build config inline (mirrors server-side buildConfig)
                            const kv = Object.fromEntries(
                                          (configQuery.data ?? []).map((r) => [r.key, r.value ?? ''])
                                        )
