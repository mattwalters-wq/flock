...
export async function POST(request) {
  try {
    ...
    let page = 1;
    while (true) {
      const { data: usersPage, error: listErr } = await db.auth.admin.listUsers({ page, perPage: 1000 });
      if (listErr) { console.error('[band-post] listUsers error:', listErr); break; }
      if (!usersPage?.users?.length) break;
      usersPage.users.forEach(u => { if (userIds.has(u.id)) emailMap[u.id] = u.email; });
      if (usersPage.users.length < 1000) break;
      page++;
    }

    const emails = [...userIds].map(id => emailMap[id]).filter(Boolean);
    if (emails.length === 0) return NextResponse.json({ ok: true, sent: 0, note: 'no emails resolved' });

    const APP_DOMAIN = process.env.NEXT_PUBLIC_APP_DOMAIN || 'fans-flock.com';
    const communityUrl = `https://${tenant.slug}.${APP_DOMAIN}`;
    const subject = `new post from ${authorName?.toLowerCase() || tenant.name.toLowerCase()} \u2736`;
    const from = `${tenant.name} <hello@fans-flock.com>`;
    const html = `
      <div style="font-family:'DM Sans',sans-serif;max-width:480px;margin:0 auto;background:#F5EFE6;padding:32px 24px;border-radius:12px;">
        ...
        <div style="background:#FAF5F0;border-radius:10px;padding:20px;border:1px solid #E8DDD4;margin-bottom:20px;">
          ...
          <p style="font-size:14px;color:#1A1018;line-height:1.6;margin:0;">${content?.slice(0, 300)}${(content?.length || 0) > 300 ? '...' : ''}</p>
        </div>
        ...
        <div style="font-family:'DM Mono',monospace;font-size:9px;color:#6A5A62;letter-spacing:1.5px;margin-top:24px;padding-top:16px;border-top:1px solid #E8DDD4;">powered by flock</div>
      </div>
    `;
    ...
    for (let i = 0; i < emails.length; i += 100) {
      ...
      const batch = chunk.map(email => ({ from, to: email, subject, html }));
      ...
      if (!resendRes.ok) { console.error('[band-post] Resend error:', resendData); }
