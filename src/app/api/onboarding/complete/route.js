...
export async function POST(request) {
  try {
    ...
    const { data: tenant, error: tenantError } = await db
      .from('tenants')
      .insert({ slug: community.slug, name: community.name, custom_domain: null })
      .select('id')
      .single();
    ...
    const memberList = members.actType === 'solo'
      ? [{ name: members.members[0]?.name || account.fullName, slug: (members.members[0]?.name || account.fullName).toLowerCase().replace(/[^a-z0-9]+/g, '-'), accent_color: members.members[0]?.color || branding.primaryColor, display_order: 0, tenant_id: tenantId }]
      : (members.members || []).filter(m => m.name?.trim()).map((m, i) => ({ name: m.name.trim(), slug: m.name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-'), accent_color: m.color || branding.primaryColor, display_order: i, tenant_id: tenantId }));

    if (memberList.length > 0) {
      await db.from('tenant_members').insert(memberList);
    }
