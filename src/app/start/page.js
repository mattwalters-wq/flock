            ...
            <div>
              ...
              {[
                ...
                <div key={key} style={{ marginBottom: 14 }}>
                  ...
                  <input type={type} value={account[key]} onChange={e => setAccount(p => ({ ...p, [key]: e.target.value }))} placeholder={placeholder}
          ...
          {step === 2 && (
            <div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input type="text" value={community.name} onChange={e => handleNameChange(e.target.value)} placeholder="e.g. The Stamps, Emma Donovan"
                  style={{ width: '100%', padding: '11px 14px', background: CREAM, border: `1px solid ${BORDER}`, borderRadius: 8, fontSize: 13, color: INK, outline: 'none', fontFamily: "'DM Sans', sans-serif", boxSizing: 'border-box' }} />
              </div>
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <input type="text" value={community.slug} onChange={e => { setCommunity(p => ({ ...p, slug: slugify(e.target.value) })); setSlugAvailable(null); }} onBlur={() => community.slug && checkSlug(community.slug)} placeholder="your-artist-name"
                    style={{ flex: 1, padding: '11px 14px', background: CREAM, border: `1px solid ${slugAvailable === false ? RUBY : slugAvailable === true ? '#7D8B6A' : BORDER}`, borderRadius: 8, fontSize: 13, color: INK, outline: 'none', fontFamily: "'DM Mono', monospace", boxSizing: 'border-box' }} />
                  <button onClick={() => checkSlug(community.slug)} disabled={!community.slug || checkingSlug} style={{ padding: '11px 14px', background: BORDER, color: SLATE, border: 'none', borderRadius: 8, fontSize: 11, cursor: 'pointer', fontFamily: "'DM Mono', monospace'" }}>
                    {checkingSlug ? '...' : 'check'}
                  </button>
                </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <input type="text" value={community.tagline} onChange={e => setCommunity(p => ({ ...p, tagline: e.target.value }))} placeholder="a short description of your community"
                  style={{ width: '100%', padding: '11px 14px', background: CREAM, border: `1px solid ${BORDER}`, borderRadius: 8, fontSize: 13, color: INK, outline: 'none', fontFamily: "'DM Sans', sans-serif", boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => { if (!community.name || !community.slug) { setError('name and slug required'); return; } if (slugAvailable === false) { setError('that slug is taken'); return; } setError(''); setStep(3); }} style={{ flex: 1, padding: '13px', background: RUBY, color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                  continue →
                </button>
              </div>
          ...
          {step === 3 && (
            <div>
                ...
                <div key={key} style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    <input type="color" value={branding[key]} onChange={e => setBranding(p => ({ ...p, [key]: e.target.value }))}
                      style={{ width: 56, height: 44, padding: 3, border: `1px solid ${BORDER}`, borderRadius: 8, cursor: 'pointer' }} />
                    <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => setStep(4)} style={{ flex: 1, padding: '13px', background: RUBY, color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                  continue →
                </button>
              </div>
            </div>
          ...
          {step === 4 && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
                {membersData.members.map((m, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <input type="text" value={m.name} onChange={e => setMembersData(p => ({ ...p, members: p.members.map((x, j) => j === i ? { ...x, name: e.target.value } : x) }))} placeholder={membersData.actType === 'solo' ? 'your name' : `member ${i + 1} name`}
                      style={{ flex: 1, padding: '9px 12px', background: CREAM, border: `1px solid ${BORDER}`, borderRadius: 8, fontSize: 13, color: INK, outline: 'none', fontFamily: "'DM Sans', sans-serif'" }} />
                    <input type="color" value={m.color} onChange={e => setMembersData(p => ({ ...p, members: p.members.map((x, j) => j === i ? { ...x, color: e.target.value } : x) }))}
                      style={{ width: 40, height: 36, padding: 2, border: `1px solid ${BORDER}`, borderRadius: 6, cursor: 'pointer' }} />
              ...
              {membersData.actType === 'band' && membersData.members.length < 6 && (
                <button onClick={() => setMembersData(p => ({ ...p, members: [...p.members, { name: '', color: '#888888' }] }))} style={{ background: 'none', border: `1px dashed ${BORDER}`, borderRadius: 8, padding: '8px', width: '100%', cursor: 'pointer', fontFamily: "'DM Mono', monospace", fontSize: 11, color: SLATE, marginBottom: 16 }}>
                  + add member
                </button>
              )}
              {error && <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: RUBY, marginBottom: 12 }}>{error}</div>}
