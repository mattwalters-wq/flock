...
export async function getTenantBySlug(slug) {
    ...
    const { data: tenant } = await db.from('tenants').select('*').eq('slug', slug).single();
    ...
    const result = { ...tenant, config, members: membersResult.data || [] };
...
export async function getTenantByDomain(domain) {
    const db = getServiceSupabase();
    const { data: tenant } = await db.from('tenants').select('slug').eq('custom_domain', domain).single();
    if (!tenant) return null;
...
export function getTenantMemberMap(tenant) {
    ...
    (tenant?.members || []).forEach(m => {
          map[m.slug] = { name: m.name, slug: m.slug, accentColor: m.accent_color || '#888888', bio: m.bio || '', displayOrder: m.display_order || 0 };
    });
    return map;
}
